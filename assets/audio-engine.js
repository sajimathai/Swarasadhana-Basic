  ...
  startMridangam() {
    ...
    const tick = () => {
      ...
      while (m.next < this._now() + 0.25) {
        ...
        if (!isAnd && typeof m.onBeat === "function") {
          const beat = Math.floor((m.idx % pat.length) / subPerBeat);
          const total = th.beats;
          const delay = Math.max(0, (m.next - this._now()) * 1000);
          setTimeout(() => { if (m.running) m.onBeat(beat, accent, total); }, delay);
        }
        if (isAnd && !m.andSound) { m.idx++; m.next += subDur; continue; }
        // ring the cymbal at the head of every cycle
        if (accent) this._cymbal(m.next, m.gain * 1.5);
        if (slot === "B") this._perc(m.next, "B", m.gain * (accent ? 1.9 : 1.6), m.inst);
        else if (slot === "T") this._perc(m.next, "T", m.gain * 1.5, m.inst);
        else if (slot === "K") this._perc(m.next, "K", m.gain * 1.35, m.inst);
        m.idx++;
        m.next += subDur;
      }
      m.timer = setTimeout(tick, 25);
  ...
  sarali: {
    ...
    items: [
      ...
      { id: "s4", name: "സരളി വരിശ 4", latin: "Four-note groups", group: 8,
        notes: [0,1,4,5,7,"h",0,1,0,1,4,5,7,8,11,12,12,11,8,7,5,"h",12,11,12,11,8,7,5,4,1,0],
